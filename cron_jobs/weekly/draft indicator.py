...
def generate_renko_signal_direction_v2(prices, box_size):
    ...
    for price in prices:
        ...
        if abs(brick_count) >= 1:
            ...
            if current_trend == 'نزولی' and direction == 'صعودی':
...
def build_weekly_indicators():
    ...
    for symbol in df['stock_ticker'].unique():
        ...
        data['signal_macd_buy_d'] = np.where((data['macd_d'].shift(1) < data['macd_signal_d'].shift(1)) &
                                             (data['macd_d'] > data['macd_signal_d']), 1, 0)
        data['signal_macd_sell_d'] = np.where((data['macd_d'].shift(1) > data['macd_signal_d'].shift(1)) &
                                              (data['macd_d'] < data['macd_signal_d']), 1, 0)
    ...
    insert_query = """
    INSERT INTO weekly_indicators_test (
        stock_ticker, week_end,
        ema_20, ema_50, ema_100,
        rsi, macd, macd_signal, macd_hist,
        tenkan, kijun, senkou_a, senkou_b, chikou,
